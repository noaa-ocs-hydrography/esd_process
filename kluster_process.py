...
def run_kluster(multibeam_files: list, outfold: str = None, logger: logging.Logger = None):
    ...
    try:
        _, converted_data_list = run_kluster_intel_process(multibeam_files, outfold)
    except:
        converted_data_list = []
    if logger:
    ...
    try:
        surf, export_path = build_kluster_surface(converted_data_list, outfold)
    except:
...
def run_kluster_intel_process(multibeam_files: list, outfold: str = None):
    """
...
def build_kluster_surface(converted_data_list: list, outfold: str = None):
    """
